...
let colors = ["green", "yellowgreen", "coral", "blue"];
...
//calling function to presist data on UI
loadTasks();
...
<div class="modal-priority-section">
  <div class="priority-inner-container">
    ...
    <div class="modal-priority yellowgreen"></div>
    <div class="modal-priority coral"></div>
    <div class="modal-priority blue selected"></div>
  </div>
</div>`;
  ...
  taskInnerContainer.addEventListener("keydown", function (e) {
    if (e.key == "Enter") {
      //generating new id
      let id=uid();
      //getting that current target having that id
      let task=e.currentTarget.innerText;

      //step1=>jobhi data local storage pr hai usse lekr aao
      //using parse int coz sting ko object form mein krnge
      let allTickets = JSON.parse(localStorage.getItem("AllTickets"));

      //step2=> usko update kro
      //object ko update kro
      let ticketObj = {
        color: ticketColor,
        taskValue: task,
      };
      ...
      //using stringify coz obj ko string ki form mein convert kr rhe coz local storage
      //mein string ki form mein save hota hai
      ...
      ticketDiv.innerHTML = ` <div data-id="${id}" class="ticket-color ${ticketColor}"></div>
        ...
        actualTaskDiv.addEventListener("input", function (e) {
          //those 3 steps to update and save changes in local storage
        ...
        //those 3 steps to update and save ticket color in local storage
      ...
      ticketDiv.addEventListener("click", function (e) {
        if (deleteMode) {
          //if ticket is deleted then update and also deleting it 
          //from local storage
          let currTicketId = e.currentTarget.getAttribute("data-id");
...
//function to persist data on UI
function loadTasks(color) {
  ...
  for (x in allTickets) {
    let currTicketId = x;
    let singleTicketObj = allTickets[x]; //green

    //passed color was blue
    if (color) {
